...
make_x1 <- function(df, latlon = NULL, groups, allele_list = ggg_allele_list,
                    out_of_place = NULL, lat = "lat", lon = "lon",
                    shiny = NULL, ...){
  ...
  if(!is.null(shiny)){
    ...
    df_c <- map(seq_along(df_c), ~ {
                  updateProgressBar(session = shiny$session, id = shiny$id, value = shiny$start + .x, title = shiny$title);
...
admix_dbs <- function(dbs, tol = 1e-8, pops = NULL, no_cores = NULL, shiny = NULL){
  ...
  if(!is.null(shiny)){
    dbs <- dbs %>% mutate(data = imap(data, ~ {
      updateProgressBar(session = shiny$session, id = shiny$id, value = shiny$start + as.integer(.y), title = shiny$title)
