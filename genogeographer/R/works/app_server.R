...
server_api <- function(input, output, session){
  ...
  reactive_read_data <- reactive({
    ...
    if(grepl("xls", ext)) data <- rio::import(input$dataset_file$datapath) %>% as_tibble()
    else data <- rio::import(input$dataset_file$datapath, header = TRUE) %>% as_tibble()
    ## Remove NA columns
    data <- data %>% select(which(map_lgl(.x = ., .f = ~ !all(is.na(.x)))))
    ndata <- names(data)
    ## Update columns:
  ...
  reactive_read_info <- reactive({
    ...
    if(grepl("xls", ext)) data <- rio::import(input$info_file$datapath) %>% as_tibble()
    else data <- rio::import(input$info_file$datapath, header = TRUE) %>% as_tibble()
    ## Remove NA columns
    data <- data %>% select(which(map_lgl(.x = ., .f = ~ !all(is.na(.x)))))
    ndata <- names(data)
    ## Update columns:
  ...
  reactive_dataset <- reactiveValues(columns = NULL, rs = NULL, success = NULL)
  reactive_info <- reactiveValues(columns = NULL, outofplace = NULL, success = NULL)
  ...
  output$info_data <- renderUI({
    if(!is.null(reactive_dataset$success) & !is.null(reactive_info$success)){
      ...
      verticalLayout(
        ...
        progressBar(id = "db_progress", value = 0, total = n_comp, title = "Computation process", display_pct = TRUE),
        downloadButton(outputId = "download_x1", label = "Create and download databases", icon = icon("database"))
      )
    }
    else tags$p("")
  })
  ...
  db_x1 <- function(){
    ...
    info_ <- reactive_selected_info() %>%
      rename(pop = input$col_POP, population = input$col_population,
             meta = input$col_META, metapopulation = input$col_metapopulation,
             lat = input$col_lat, lon = input$col_lon)
    n_comp <- cumsum(c(n_pop, n_meta, choose(n_pop, 2), choose(n_meta, 2)))
    ##
    shiny_pre <- "Computation process:"
    pop_DB <- make_x1(df = data_, latlon = info_, groups = "pop", exclude = c("sample", "meta"),
                      shiny = list(session = session, id = "db_progress", start = 0, title = paste(shiny_pre, "Population databases")))
    meta_DB <- make_x1(df = data_, latlon = info_, groups = "meta", exclude = c("sample", "pop"),
                       out_of_place = input$col_outofplace,
                       shiny = list(session = session, id = "db_progress", start = n_comp[1], title = paste(shiny_pre, "Metapopulation databases")))
    pop_DB_1admix <- admix_dbs(pop_DB,
                               shiny = list(session = session, id = "db_progress", start = n_comp[2], title = paste(shiny_pre, "Population admixtures")))
    meta_DB_1admix <- admix_dbs(meta_DB,
                                shiny = list(session = session, id = "db_progress", start = n_comp[3], title = paste(shiny_pre, "Metapopulation admixtures")))
